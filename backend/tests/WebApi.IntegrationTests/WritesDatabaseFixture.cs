...
public sealed class WritesDatabaseFixture : IAsyncLifetime
{
    private static readonly Lazy<WritesDatabaseFixture> LazyInstance = new(() => new WritesDatabaseFixture());

    private readonly MySqlContainer container = new MySqlBuilder()
        .WithImage("mysql:8.0")
        .Build();

    private WritesDatabaseFixture()
    {
    }

    public static WritesDatabaseFixture Instance => LazyInstance.Value;
