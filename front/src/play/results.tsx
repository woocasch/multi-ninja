...
export default function ResultsComponent(props: Properties) {
    const answersToShow = useMemo(() => props.answeredQuestions.filter((v => v.providedAnswers.length > 0)), [props.answeredQuestions]);
    ...
    const hasNext = useMemo(() => currentIndex < totalAnswers - 1, [currentIndex, totalAnswers]);
    ...
    function responseClassNames(answer: number, question: Model.AnsweredQuestion) {
        let className = '';
        ...
        <div className='results'>
            ...
            <div className="answer">
                {currentItem.question.answerPropositions?.map((v, i) => (
                    <button key={i} className={responseClassNames(v, currentItem)}>{v}</button>
                ))}
            </div>
            <div className='browser'>
                <button className='button' onClick={showPrev} disabled={!hasPrevious}>&lt;</button>
                {currentIndex + 1}
                <button className='button' onClick={showNext} disabled={!hasNext}>&gt;</button>
            </div>
        </div>
    )
}